...
QJsonObject startBlack(VibeCutTools *tools, const QJsonObject &input)
{
    ...
    const QString filter = QStringLiteral("blackdetect=d=%1:pix_th=%2")
                               .arg(minDurationMs / 1000.0, 0, 'f', 3)
                               .arg(pixelThreshold, 0, 'f', 3);
    ...
    QObject::connect(process, qOverload<int, QProcess::ExitStatus>(&QProcess::finished), tools,
        ...
        if (job.state == VibeCutJobState::CancelRequested) {
            jobs->markCancelled(jobId, QStringLiteral("Black-frame detection cancelled."));
            process->deleteLater();
            return;
        }
        const QString stderrText = QString::fromUtf8(process->readAllStandardError());
        if (exitStatus != QProcess::NormalExit || exitCode != 0) {
            jobs->markFailed(jobId, QStringLiteral("FFmpeg blackdetect failed (exit %1).").arg(exitCode));
            process->deleteLater();
            return;
        }
        ...
        for (const auto &range : ranges) {
            ...
            record.metadata = QJsonObject{{QStringLiteral("start_seconds"), range.first},
                                          ...
                                          {QStringLiteral("pixel_threshold"), pixelThreshold}};
        ...
        if (!VibeCutMediaEvidence::replaceSourceExtractorCurrent(QStringLiteral("bin:%1").arg(binId), fingerprint,
                                                                  ...
                                                                  records, &error)) {
            jobs->markFailed(jobId, QStringLiteral("Black-frame evidence persistence failed: %1").arg(error));
            process->deleteLater();
            return;
        }
    ...
    QObject::connect(process, &QProcess::errorOccurred, tools, [jobs, process, jobId](QProcess::ProcessError processError) {
        ...
        if (jobs->job(jobId, job) && !job.terminal()) {
            jobs->markFailed(jobId, QStringLiteral("Could not start/run FFmpeg blackdetect: %1").arg(process->errorString()));
        }
    });
    ...
    return QJsonObject{{QStringLiteral("ok"), true}, {QStringLiteral("job_id"), jobId},
                       ...
                       {QStringLiteral("min_duration_ms"), minDurationMs}, {QStringLiteral("pixel_threshold"), pixelThreshold},
...
bool registerVibeCutBlackExtractorTools(VibeCutToolSurface &surface, QString *error)
{
    ...
    if (!tools) {
        if (error) *error = QStringLiteral("Black-frame extractor requires the native VibeCutTools/JobManager surface.");
        return false;
    }
    const QJsonObject input{{QStringLiteral("type"), QStringLiteral("object")},
                            {QStringLiteral("properties"), QJsonObject{
                                ...
                                {QStringLiteral("min_duration_ms"), QJsonObject{{QStringLiteral("type"), QStringLiteral("integer")}, {QStringLiteral("minimum"), 20}, {QStringLiteral("maximum"), 60000}}},
                                {QStringLiteral("pixel_threshold"), QJsonObject{{QStringLiteral("type"), QStringLiteral("number")}, {QStringLiteral("minimum"), 0.0}, {QStringLiteral("maximum"), 1.0}}}}},
                            {QStringLiteral("required"), QJsonArray{QStringLiteral("bin_id")}},
                            {QStringLiteral("additionalProperties"), false}};
    const QJsonObject schema{{QStringLiteral("name"), QStringLiteral("media_black_refresh")},
                             {QStringLiteral("description"), QStringLiteral("Asynchronously run Kdenlive's configured FFmpeg blackdetect on one file-backed video bin asset, then persist black-frame ranges as versioned extractor evidence. Returns a shared VibeCut job id and supports job_cancel.")},
                             {QStringLiteral("input_schema"), input}};
    VibeCutToolPolicy policy;
    policy.name = QStringLiteral("media_black_refresh");
