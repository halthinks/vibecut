...
QList<QPair<double, double>> parseFreezeRanges(const QString &output)
{
    ...
    for (const QString &line : output.split(QLatin1Char('\n'))) {
        ...
        if (startMatch.hasMatch()) { pending = startMatch.captured(1).toDouble(); continue; }
...
QJsonObject startFreeze(VibeCutTools *tools, const QJsonObject &input)
{
    ...
    const QString filter = QStringLiteral("freezedetect=n=%1dB:d=%2").arg(noiseDb, 0, 'f', 1).arg(minDurationMs / 1000.0, 0, 'f', 3);
    ...
    QObject::connect(process, qOverload<int, QProcess::ExitStatus>(&QProcess::finished), tools,
        ...
        VibeCutJob job;
        jobs->job(jobId, job);
        if (job.state == VibeCutJobState::CancelRequested) { jobs->markCancelled(jobId, QStringLiteral("Freeze detection cancelled.")); process->deleteLater(); return; }
        const QString stderrText = QString::fromUtf8(process->readAllStandardError());
        if (exitStatus != QProcess::NormalExit || exitCode != 0) { jobs->markFailed(jobId, QStringLiteral("FFmpeg freezedetect failed (exit %1).").arg(exitCode)); process->deleteLater(); return; }
        ...
        for (const auto &range : ranges) {
            ...
            record.metadata = QJsonObject{{QStringLiteral("start_seconds"), range.first}, {QStringLiteral("end_seconds"), range.second}, {QStringLiteral("duration_seconds"), duration}, {QStringLiteral("min_duration_ms"), minDurationMs}, {QStringLiteral("noise_db"), noiseDb}};
        ...
        if (!VibeCutMediaEvidence::replaceSourceExtractorCurrent(QStringLiteral("bin:%1").arg(binId), fingerprint, QString::fromLatin1(ExtractorId), QString::fromLatin1(ExtractorVersion), records, &error)) {
            jobs->markFailed(jobId, QStringLiteral("Freeze evidence persistence failed: %1").arg(error)); process->deleteLater(); return;
        }
    ...
    QObject::connect(process, &QProcess::errorOccurred, tools, [jobs, process, jobId](QProcess::ProcessError processError) {
        ...
        VibeCutJob job;
        if (jobs->job(jobId, job) && !job.terminal()) jobs->markFailed(jobId, QStringLiteral("Could not start/run FFmpeg freezedetect: %1").arg(process->errorString()));
    });
    process->start(ffmpeg, args);
    return QJsonObject{{QStringLiteral("ok"), true}, {QStringLiteral("job_id"), jobId}, {QStringLiteral("bin_id"), binId}, {QStringLiteral("source_fingerprint"), fingerprint}, {QStringLiteral("extractor_id"), QString::fromLatin1(ExtractorId)}, {QStringLiteral("extractor_version"), QString::fromLatin1(ExtractorVersion)}, {QStringLiteral("min_duration_ms"), minDurationMs}, {QStringLiteral("noise_db"), noiseDb}, {QStringLiteral("asynchronous"), true}};
}
} // namespace
...
bool registerVibeCutFreezeExtractorTools(VibeCutToolSurface &surface, QString *error)
{
    ...
    if (!tools) { if (error) *error = QStringLiteral("Freeze extractor requires the native VibeCutTools/JobManager surface."); return false; }
    const QJsonObject input{{QStringLiteral("type"), QStringLiteral("object")}, {QStringLiteral("properties"), QJsonObject{{QStringLiteral("bin_id"), QJsonObject{{QStringLiteral("type"), QStringLiteral("string")}}}, {QStringLiteral("min_duration_ms"), QJsonObject{{QStringLiteral("type"), QStringLiteral("integer")}, {QStringLiteral("minimum"), 100}, {QStringLiteral("maximum"), 600000}}}, {QStringLiteral("noise_db"), QJsonObject{{QStringLiteral("type"), QStringLiteral("number")}, {QStringLiteral("minimum"), -100}, {QStringLiteral("maximum"), 0}}}}}, {QStringLiteral("required"), QJsonArray{QStringLiteral("bin_id")}}, {QStringLiteral("additionalProperties"), false}};
    const QJsonObject schema{{QStringLiteral("name"), QStringLiteral("media_freeze_refresh")}, {QStringLiteral("description"), QStringLiteral("Asynchronously run Kdenlive's configured FFmpeg freezedetect on one file-backed video bin asset, then persist frozen-video ranges as versioned extractor evidence. Returns a shared VibeCut job id and supports job_cancel.")}, {QStringLiteral("input_schema"), input}};
    VibeCutToolPolicy policy;
    policy.name = QStringLiteral("media_freeze_refresh"); policy.risk = VibeCutToolRisk::ExternalSideEffect; policy.asynchronous = true; policy.mutatesProject = false;
