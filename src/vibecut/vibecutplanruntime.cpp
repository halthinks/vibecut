...
{
    switch (risk) {
    case VibeCutToolRisk::ReadOnly:
        return QStringLiteral("read");
    case VibeCutToolRisk::ReversibleEdit:
        return QStringLiteral("reversible edit");
    case VibeCutToolRisk::MajorEdit:
        return QStringLiteral("major edit");
    case VibeCutToolRisk::ExternalSideEffect:
        return QStringLiteral("external side effect");
    case VibeCutToolRisk::Irreversible:
...
{
    if (!m_surface) {
        return err(QStringLiteral("Plan runtime has no tool surface."));
    }
    ...
    QString error;
    if (!setPendingPlan(plan, error)) {
        return err(error);
    }
    return QJsonObject{{QStringLiteral("ok"), true},
                       {QStringLiteral("awaiting_approval"), true},
...
QJsonObject VibeCutPlanRuntime::proposeDirectToolCalls(const QJsonArray &toolUseBlocks, const QString &objective)
{
    ...
    for (const QJsonValue &value : toolUseBlocks) {
        ...
        if (!previousId.isEmpty()) {
            operation.dependsOn.append(previousId);
        }
    ...
    if (plan.operations.isEmpty()) {
        return err(QStringLiteral("No mutating or side-effecting tool calls were present to review."));
    }
...
bool VibeCutPlanRuntime::setPendingPlan(const VibeCutEditPlan &plan, QString &error)
{
    ...
    for (const VibeCutPlanOperation &operation : plan.operations) {
        ...
        if (policy.value().risk != VibeCutToolRisk::ReadOnly) {
            hasEffect = true;
        }
    }
    if (!hasEffect) {
...
QJsonObject VibeCutPlanRuntime::approvePendingPlan()
{
    if (!m_hasPending || m_executing) {
        return err(QStringLiteral("There is no reviewable VibeCut plan awaiting approval."));
    }
    if (!m_surface) {
        return err(QStringLiteral("Plan runtime has no tool surface."));
    }

    const VibeCutPlanGateResult gate = VibeCutPlanGate::assess(m_plan, m_surface->projectRevision(), m_surface->policies(),
                                                               VibeCutTrustMode::Off, true);
    if (!gate.ready()) {
    ...
    return QJsonObject{{QStringLiteral("ok"), true},
                       ...
                       {QStringLiteral("waiting_job_id"), m_waitingJobId}};
}

QJsonObject VibeCutPlanRuntime::cancelPendingPlan()
{
    if (!m_hasPending) {
        return err(QStringLiteral("There is no pending plan to cancel."));
    }
    if (m_executing) {
        return err(QStringLiteral("The plan is already executing; cancellation of active editor operations is not yet supported."));
    }
...
{
    if (!m_executing || !m_surface || !m_waitingJobId.isEmpty()) {
        return;
    }

    const QHash<QString, VibeCutToolPolicy> policies = m_surface->policies();
    while (m_executionIndex < m_executionOrder.size()) {
        if (m_surface->projectRevision() != m_expectedRevision) {
        ...
        Q_EMIT planProgress(QStringLiteral("Step %1/%2: %3…")
                                .arg(m_executionIndex + 1)
                                .arg(m_executionOrder.size())
        ...
        if (!result.value(QStringLiteral("ok")).toBool()) {
            finishExecution(false, QStringLiteral("Step %1 failed: %2")
        ...
        if (result.value(QStringLiteral("started")).toBool()) {
            ...
            if (jobId.isEmpty()) {
                finishExecution(false,
                                QStringLiteral("Step %1 started asynchronously without a trackable job id; the plan was stopped rather than guessing when to continue.")
                                    .arg(operation->toolName));
                return;
            }
    ...
    finishExecution(true, QStringLiteral("Approved plan completed and every synchronous checkpoint reported success."));
}

void VibeCutPlanRuntime::onJobChanged(const QString &jobId)
{
    if (!m_executing || jobId != m_waitingJobId || !m_surface || !m_surface->baseTools()) {
        return;
    }

    VibeCutJob job;
    if (!m_surface->baseTools()->jobManager()->job(jobId, job) || !job.terminal()) {
        return;
    }
    ...
    // A mutating async VibeCut job must protect its own base revision before
    // committing (the subtitle pipeline does). Once it reports succeeded, its
    // resulting project revision becomes the new expected checkpoint.
    m_expectedRevision = m_surface->projectRevision();
    m_waitingJobId.clear();
    ++m_executionIndex;
    continueExecution();
...
{
    const QString planId = m_plan.id;
    const QJsonArray results = m_operationResults;
    m_executing = false;
    m_hasPending = false;
    m_executionOrder.clear();
    m_executionIndex = 0;
    ...
    m_plan = VibeCutEditPlan();
    m_operationResults = QJsonArray();
    Q_EMIT planFinished(planId, success, summary, results);
}
