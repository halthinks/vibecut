...
namespace {
constexpr auto ExtractorId = "silence_detect";
...
QList<QPair<double, double>> parseSilence(const QString &stderrText)
{
    QList<QPair<double, double>> ranges;
    const QRegularExpression startRe(QStringLiteral("silence_start:\\s*([0-9]+(?:\\.[0-9]+)?)"));
    const QRegularExpression endRe(QStringLiteral("silence_end:\\s*([0-9]+(?:\\.[0-9]+)?)"));
    ...
    for (const QString &line : lines) {
        ...
        if (endMatch.hasMatch() && pendingStart >= 0.0) {
            const double end = endMatch.captured(1).toDouble();
            if (end >= pendingStart) ranges.append({pendingStart, end});
            pendingStart = -1.0;
        }
    }
    return ranges;
...
QJsonObject startSilence(VibeCutTools *tools, const QJsonObject &input)
{
    ...
    const int minDurationMs = qBound(50, input.value(QStringLiteral("min_duration_ms")).toInt(500), 60000);
    const double noiseDb = qBound(-100.0, input.value(QStringLiteral("noise_db")).toDouble(-35.0), -1.0);
    ...
    if (!clip->hasUrl()) return err(QStringLiteral("Silence detection requires a file-backed source."));
    ...
    const QString jobId = jobs->createJob(QStringLiteral("media_silence"),
                                          QStringLiteral("Detect silence in %1").arg(info.fileName()), true);
    jobs->markRunning(jobId, QStringLiteral("FFmpeg silencedetect is starting."));

    QProcess *process = new QProcess(tools);
    process->setProcessChannelMode(QProcess::SeparateChannels);
    const QString filter = QStringLiteral("silencedetect=noise=%1dB:d=%2")
                               .arg(noiseDb, 0, 'f', 1)
    ...
    QObject::connect(process, qOverload<int, QProcess::ExitStatus>(&QProcess::finished), tools,
                     [jobs, process, jobId, binId, fingerprint, fps, noiseDb, minDurationMs](int exitCode, QProcess::ExitStatus exitStatus) {
        VibeCutJob job;
        jobs->job(jobId, job);
        if (job.state == VibeCutJobState::CancelRequested) {
            jobs->markCancelled(jobId, QStringLiteral("Silence detection cancelled."));
            process->deleteLater();
            return;
        }
        const QString stderrText = QString::fromUtf8(process->readAllStandardError());
        if (exitStatus != QProcess::NormalExit || exitCode != 0) {
            jobs->markFailed(jobId, QStringLiteral("FFmpeg silencedetect failed (exit %1).").arg(exitCode));
            process->deleteLater();
            return;
        }

        const QList<QPair<double, double>> ranges = parseSilence(stderrText);
        QList<VibeCutMediaEvidenceRecord> records;
        int index = 0;
        for (const auto &range : ranges) {
            VibeCutMediaEvidenceRecord record;
            record.id = QStringLiteral("silence:%1:%2:%3").arg(binId).arg(fingerprint.left(12)).arg(index++);
            ...
            record.startFrame = qMax(0, qRound64(range.first * fps));
            record.endFrame = qMax(record.startFrame, qRound64(range.second * fps));
            const double duration = qMax(0.0, range.second - range.first);
            record.text = QStringLiteral("silence dead air %1 seconds").arg(duration, 0, 'f', 3);
        ...
        if (!VibeCutMediaEvidence::replaceSourceExtractorCurrent(QStringLiteral("bin:%1").arg(binId), fingerprint,
                                                                  ...
                                                                  records, &error)) {
            jobs->markFailed(jobId, QStringLiteral("Silence evidence persistence failed: %1").arg(error));
            process->deleteLater();
            return;
        }
        jobs->markSucceeded(jobId, QStringLiteral("Detected %1 silence range(s) and persisted evidence.").arg(records.size()));
        process->deleteLater();
    });
    ...
    return QJsonObject{{QStringLiteral("ok"), true}, {QStringLiteral("job_id"), jobId},
                       ...
                       {QStringLiteral("noise_db"), noiseDb}, {QStringLiteral("min_duration_ms"), minDurationMs},
                       {QStringLiteral("asynchronous"), true}};
}
} // namespace

bool registerVibeCutSilenceExtractorTools(VibeCutToolSurface &surface, QString *error)
{
    VibeCutTools *tools = surface.baseTools();
    if (!tools) {
        if (error) *error = QStringLiteral("Silence extractor requires the native VibeCutTools/JobManager surface.");
        return false;
    }
    const QJsonObject input{{QStringLiteral("type"), QStringLiteral("object")},
                            {QStringLiteral("properties"), QJsonObject{
                                ...
                                {QStringLiteral("noise_db"), QJsonObject{{QStringLiteral("type"), QStringLiteral("number")}, {QStringLiteral("minimum"), -100}, {QStringLiteral("maximum"), -1}}},
                                {QStringLiteral("min_duration_ms"), QJsonObject{{QStringLiteral("type"), QStringLiteral("integer")}, {QStringLiteral("minimum"), 50}, {QStringLiteral("maximum"), 60000}}}}},
                            {QStringLiteral("required"), QJsonArray{QStringLiteral("bin_id")}},
                            {QStringLiteral("additionalProperties"), false}};
    const QJsonObject schema{{QStringLiteral("name"), QStringLiteral("media_silence_refresh")},
                             {QStringLiteral("description"), QStringLiteral("Asynchronously run Kdenlive's configured FFmpeg silencedetect on one file-backed audio/video bin asset, then persist exact silence/dead-air frame ranges as versioned extractor evidence. Returns a shared VibeCut job id and supports job_cancel.")},
                             {QStringLiteral("input_schema"), input}};
    VibeCutToolPolicy policy;
    policy.name = QStringLiteral("media_silence_refresh");
    policy.risk = VibeCutToolRisk::ExternalSideEffect;
    policy.asynchronous = true;
    policy.mutatesProject = false;
    return surface.registerTool(schema, policy, [tools](const QJsonObject &input) { return startSilence(tools, input); }, error);
}
