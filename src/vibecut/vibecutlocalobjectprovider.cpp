                       ...
                       double minimum, double maximum, double &result, QString *error)
{
    ...
    if (!value.isDouble()) {
        if (error) *error = QStringLiteral("Object-detection parameter %1 must be numeric.").arg(name);
        return false;
    }
    result = value.toDouble();
    if (!std::isfinite(result) || result < minimum || result > maximum) {
        if (error) *error = QStringLiteral("Object-detection parameter %1 must be between %2 and %3.")
                    ...
                    int minimum, int maximum, int &result, QString *error)
{
    ...
    if (!value.isDouble() || !std::isfinite(raw) || static_cast<double>(converted) != raw ||
        converted < minimum || converted > maximum) {
        if (error) *error = QStringLiteral("Object-detection parameter %1 must be an integer from %2 to %3.")
                               .arg(name).arg(minimum).arg(maximum);
        return false;
    }
    result = converted;
...
public:
                      ...
                      const VibeCutExtractorProviderContext &context, QString *error) override
    {
        ...
        if (capability.trimmed().toLower() != QLatin1String("objects")) {
            if (error) *error = QStringLiteral("Local DETR only implements objects.");
            return QJsonObject();
        }
        if (!context.jobs || !context.persistEvidence) {
            if (error) *error = QStringLiteral("Local DETR requires the shared JobManager and validated evidence sink.");
            return QJsonObject();
        }
        if (!input.value(QStringLiteral("has_video")).toBool(false)) {
            if (error) *error = QStringLiteral("Object detection requires a source with video.");
            return QJsonObject();
        }
        QString readyError;
        if (!configured(&readyError)) {
        ...
        if (ffmpeg.isEmpty() || !QFileInfo::exists(ffmpeg)) {
            if (error) *error = QStringLiteral("Kdenlive has no valid configured FFmpeg executable for visual sampling.");
            return QJsonObject();
        }
        ...
        if (!readBoundedInt(parameters, QStringLiteral("sample_interval_frames"), 30, 1, 1000000, sampleIntervalFrames, error) ||
            ...
            !readBoundedDouble(parameters, QStringLiteral("min_score"), 0.70, 0.0, 1.0, minScore, error)) {
            return QJsonObject();
        }
        ...
        if (device != QLatin1String("auto") && device != QLatin1String("cpu") && device != QLatin1String("cuda")) {
            if (error) *error = QStringLiteral("Object-detection parameter device must be auto, cpu, or cuda.");
            return QJsonObject();
        }
        const qint64 requiredSamples = 1 + (static_cast<qint64>(endFrame) - startFrame - 1) / sampleIntervalFrames;
        if (requiredSamples < 1 || requiredSamples > maxSamples) {
            if (error) *error = QStringLiteral("Object-detection range/cadence requires %1 sampled frames, exceeding max_samples=%2; increase sample_interval_frames or use a smaller range.")
                                   .arg(requiredSamples).arg(maxSamples);
            return QJsonObject();
        }

        const QStringList arguments{
            vibeCutObjectDetectionScript(),
            ...
            QStringLiteral("--sample-interval-frames"), QString::number(sampleIntervalFrames),
            QStringLiteral("--max-samples"), QString::number(maxSamples),
            QStringLiteral("--min-score"), QString::number(minScore, 'f', 6),
            QStringLiteral("--max-detections-per-frame"), QString::number(maxDetections),
            QStringLiteral("--device"), device,
            QStringLiteral("--model"), kModel,
            QStringLiteral("--revision"), kModelRevision,
        };

        const QString jobId = context.jobs->createJob(QStringLiteral("visual_objects"),
                                                      QStringLiteral("Object detection · %1").arg(sourceId), true);
        ...
        QObject::connect(process, qOverload<int, QProcess::ExitStatus>(&QProcess::finished), process,
                         [process, jobs = context.jobs, jobId, persistEvidence, sourceId, sourceFingerprint,
                          startFrame, endFrame, sampleIntervalFrames, requiredSamples, maxDetections, minScore]
                         (int exitCode, QProcess::ExitStatus status) {
            VibeCutJob current;
            if (jobs->job(jobId, current) && current.state == VibeCutJobState::CancelRequested) {
                jobs->markCancelled(jobId, QStringLiteral("Object detection cancelled."));
                process->deleteLater();
                return;
            }
            if (status != QProcess::NormalExit || exitCode != 0) {
                const QString stderrText = QString::fromUtf8(process->readAllStandardError()).right(6000).trimmed();
                jobs->markFailed(jobId, stderrText.isEmpty() ? QStringLiteral("Local DETR exited with code %1.").arg(exitCode) : stderrText);
                process->deleteLater();
                return;
            }
            const QByteArray stdoutData = process->readAllStandardOutput();
            if (stdoutData.size() > 64 * 1024 * 1024) {
                jobs->markFailed(jobId, QStringLiteral("Local DETR output exceeded the 64 MiB safety limit."));
                process->deleteLater();
                return;
            }
            ...
            if (parseError.error != QJsonParseError::NoError || !document.isObject()) {
                jobs->markFailed(jobId, QStringLiteral("Local DETR returned malformed JSON: %1").arg(parseError.errorString()));
                process->deleteLater();
                return;
            }
            ...
            if (root.value(QStringLiteral("schema_version")).toInt(-1) != 1 ||
                ...
                root.value(QStringLiteral("taxonomy")).toString() != kTaxonomy ||
                root.value(QStringLiteral("model")).toString() != kModel ||
                root.value(QStringLiteral("model_revision")).toString() != kModelRevision ||
                transformersVersion != kTransformersVersion || !torchVersion.startsWith(kTorchVersion) ||
                !torchvisionVersion.startsWith(kTorchvisionVersion) ||
                root.value(QStringLiteral("sample_interval_frames")).toInt(-1) != sampleIntervalFrames ||
                root.value(QStringLiteral("sample_count")).toInt(-1) != requiredSamples ||
                !root.value(QStringLiteral("samples")).isArray()) {
                jobs->markFailed(jobId, QStringLiteral("Local DETR returned an unsupported or provenance-mismatched result schema."));
                process->deleteLater();
                return;
            }
            ...
            if (samples.size() != requiredSamples) {
                jobs->markFailed(jobId, QStringLiteral("Local DETR result did not cover the exact requested sample sequence."));
                process->deleteLater();
                return;
            }
            ...
            for (int sampleIndex = 0; sampleIndex < samples.size(); ++sampleIndex) {
                if (!samples.at(sampleIndex).isObject()) {
                    jobs->markFailed(jobId, QStringLiteral("Local DETR returned a non-object sample."));
                    process->deleteLater();
                    return;
                }
                ...
                if (expectedFrame64 < startFrame || expectedFrame64 >= endFrame || expectedFrame64 > std::numeric_limits<int>::max()) {
                    jobs->markFailed(jobId, QStringLiteral("Internal DETR sample-frame calculation overflowed authoritative bounds."));
                    process->deleteLater();
                    return;
                }
                ...
                if (frame != expectedFrame || imageWidth <= 0 || imageHeight <= 0 || detections.size() > maxDetections) {
                    jobs->markFailed(jobId, QStringLiteral("Local DETR returned invalid sampled-frame metadata."));
                    process->deleteLater();
                    return;
                }
                for (const QJsonValue &detectionValue : detections) {
                    if (!detectionValue.isObject()) {
                        jobs->markFailed(jobId, QStringLiteral("Local DETR returned a non-object detection."));
                        process->deleteLater();
                        return;
                    }
                    const QJsonObject detection = detectionValue.toObject();
                    const QString label = detection.value(QStringLiteral("label")).toString().trimmed();
                    const QJsonValue labelIdValue = detection.value(QStringLiteral("label_id"));
                    const double rawLabelId = labelIdValue.toDouble(-1.0);
                    const int labelId = labelIdValue.toInt(-1);
                    const double score = detection.value(QStringLiteral("score")).toDouble(-1.0);
                    const QJsonObject box = detection.value(QStringLiteral("bbox_pixels")).toObject();
                    ...
                    if (label.isEmpty() || label.size() > 256 || !labelIdValue.isDouble() || labelId < 0 ||
                        static_cast<double>(labelId) != rawLabelId || !std::isfinite(score) || score < minScore - 0.000001 || score > 1.0 ||
                        x < 0 || y < 0 || width <= 0 || height <= 0 ||
                        static_cast<qint64>(x) + width > imageWidth || static_cast<qint64>(y) + height > imageHeight) {
                        jobs->markFailed(jobId, QStringLiteral("Local DETR returned an invalid detection record."));
                        process->deleteLater();
                        return;
                    }
                    VibeCutMediaEvidenceRecord record;
                    record.id = QStringLiteral("object:%1:%2:%3").arg(sourceId.mid(sourceId.indexOf(QLatin1Char(':')) + 1),
                                                                       sourceFingerprint.left(12)).arg(recordIndex++);
            ...
            if (!persistEvidence(sourceId, sourceFingerprint, kExtractorId, kExtractorVersion, records, &persistError)) {
                jobs->markFailed(jobId, QStringLiteral("Object-detection evidence was rejected: %1").arg(persistError));
                process->deleteLater();
                return;
            }
        ...
        QObject::connect(process, &QProcess::errorOccurred, process,
            ...
            jobs->markFailed(jobId, QStringLiteral("Could not launch the configured VibeCut vision Python environment."));
            process->deleteLater();
        });
...
void ensureVibeCutLocalObjectProviderRegistered()
{
    ...
    if (VibeCutExtractorProviderRegistry::global().registerProvider(kProviderId,
                                                                    []() { return std::make_unique<LocalDetrObjectProvider>(); },
                                                                    &error)) {
        registered = true;
        return;
    }
