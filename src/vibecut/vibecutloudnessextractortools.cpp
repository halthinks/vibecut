...
QJsonObject startLoudness(VibeCutTools *tools, const QJsonObject &input)
{
    ...
    const QString jobId = jobs->createJob(QStringLiteral("media_loudness"),
                                          QStringLiteral("Measure loudness in %1").arg(info.fileName()), true);
    ...
    const QStringList args{QStringLiteral("-hide_banner"), QStringLiteral("-nostats"),
                           ...
                           QStringLiteral("-vn"), QStringLiteral("-f"), QStringLiteral("null"), QStringLiteral("-")};
    ...
    QObject::connect(process, qOverload<int, QProcess::ExitStatus>(&QProcess::finished), tools,
        ...
        if (!parseMetric(stderrText, QStringLiteral("mean_volume"), meanVolume) ||
            !parseMetric(stderrText, QStringLiteral("max_volume"), maxVolume)) {
        ...
        record.text = QStringLiteral("audio loudness mean %1 dB max %2 dB %3")
                          .arg(meanVolume, 0, 'f', 2)
                          .arg(maxVolume, 0, 'f', 2)
                          .arg(nearClipping ? QStringLiteral("near clipping") : QStringLiteral("not clipping"));
        record.confidence = 1.0;
        record.producedUtc = QDateTime::currentDateTimeUtc().toString(Qt::ISODateWithMs);
        record.metadata = QJsonObject{{QStringLiteral("mean_volume_db"), meanVolume},
                                      {QStringLiteral("max_volume_db"), maxVolume},
                                      {QStringLiteral("clipping_threshold_db"), clippingThresholdDb},
                                      {QStringLiteral("near_clipping"), nearClipping}};
        ...
        jobs->markSucceeded(jobId, QStringLiteral("Measured mean %1 dB, max %2 dB%3.")
                                       .arg(meanVolume, 0, 'f', 2)
                                       .arg(maxVolume, 0, 'f', 2)
                                       .arg(nearClipping ? QStringLiteral(" (near clipping)") : QString()));
        process->deleteLater();
    });
    ...
    QObject::connect(process, &QProcess::errorOccurred, tools, [jobs, process, jobId](QProcess::ProcessError processError) {
        ...
        if (jobs->job(jobId, job) && !job.terminal()) {
            jobs->markFailed(jobId, QStringLiteral("Could not start/run FFmpeg volumedetect: %1").arg(process->errorString()));
        }
    });
...
bool registerVibeCutLoudnessExtractorTools(VibeCutToolSurface &surface, QString *error)
{
    ...
    const QJsonObject input{{QStringLiteral("type"), QStringLiteral("object")},
                            {QStringLiteral("properties"), QJsonObject{
                                ...
                                {QStringLiteral("clipping_threshold_db"), QJsonObject{{QStringLiteral("type"), QStringLiteral("number")}, {QStringLiteral("minimum"), -6}, {QStringLiteral("maximum"), 0}}}}},
                            {QStringLiteral("required"), QJsonArray{QStringLiteral("bin_id")}},
                            {QStringLiteral("additionalProperties"), false}};
    const QJsonObject schema{{QStringLiteral("name"), QStringLiteral("media_loudness_refresh")},
                             {QStringLiteral("description"), QStringLiteral("Asynchronously run Kdenlive's configured FFmpeg volumedetect on one file-backed audio/video bin asset, then persist measured mean/max volume and near-clipping status as versioned extractor evidence. Returns a shared VibeCut job id and supports job_cancel.")},
