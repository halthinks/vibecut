...
QJsonObject startShots(VibeCutTools *tools, const QJsonObject &input)
{
    ...
    const QString jobId = jobs->createJob(QStringLiteral("media_shots"),
                                          QStringLiteral("Detect shots in %1").arg(info.fileName()), true);
    ...
    const QStringList args{QStringLiteral("-y"), QStringLiteral("-loglevel"), QStringLiteral("info"),
                           QStringLiteral("-i"), info.absoluteFilePath(),
                           QStringLiteral("-filter:v"), filter,
    ...
    QObject::connect(process, qOverload<int, QProcess::ExitStatus>(&QProcess::finished), tools,
        ...
        for (int frame : boundaries) {
            ...
            boundary.metadata = QJsonObject{{QStringLiteral("threshold"), threshold},
                                            {QStringLiteral("min_interval_frames"), minIntervalFrames}};
        ...
        jobs->markSucceeded(jobId, QStringLiteral("Detected %1 shot boundary(ies) and %2 shot segment(s).")
                                       .arg(boundaries.size()).arg(qMax(0, cuts.size() - 1)));
        process->deleteLater();
    });
    ...
    QObject::connect(process, &QProcess::errorOccurred, tools, [jobs, process, jobId](QProcess::ProcessError processError) {
        ...
        if (jobs->job(jobId, job) && !job.terminal()) {
            jobs->markFailed(jobId, QStringLiteral("Could not start/run FFmpeg scene detection: %1").arg(process->errorString()));
        }
    });
    ...
    return QJsonObject{{QStringLiteral("ok"), true}, {QStringLiteral("job_id"), jobId},
                       ...
                       {QStringLiteral("threshold"), threshold}, {QStringLiteral("min_interval_frames"), minIntervalFrames},
...
bool registerVibeCutShotExtractorTools(VibeCutToolSurface &surface, QString *error)
{
    ...
    const QJsonObject input{{QStringLiteral("type"), QStringLiteral("object")},
                            {QStringLiteral("properties"), QJsonObject{
                                ...
                                {QStringLiteral("threshold"), QJsonObject{{QStringLiteral("type"), QStringLiteral("number")}, {QStringLiteral("minimum"), 0.01}, {QStringLiteral("maximum"), 1.0}}},
                                {QStringLiteral("min_interval_frames"), QJsonObject{{QStringLiteral("type"), QStringLiteral("integer")}, {QStringLiteral("minimum"), 0}}}}},
    ...
    const QJsonObject schema{{QStringLiteral("name"), QStringLiteral("media_shots_refresh")},
                             {QStringLiteral("description"), QStringLiteral("Asynchronously run the same FFmpeg scene-change detector used by Kdenlive on one file-backed video bin asset, then persist non-destructive shot-boundary and shot-segment frame evidence. Returns a shared VibeCut job id and supports job_cancel.")},
