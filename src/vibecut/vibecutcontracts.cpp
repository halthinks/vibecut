...
QString riskName(VibeCutToolRisk risk)
{
    ...
    case VibeCutToolRisk::ReversibleEdit:
        return QStringLiteral("reversible_edit");
    case VibeCutToolRisk::MajorEdit:
        return QStringLiteral("major_edit");
    case VibeCutToolRisk::ExternalSideEffect:
        return QStringLiteral("external_side_effect");
...
bool visitOperation(const QString &id, const QHash<QString, QStringList> &dependencies, QSet<QString> &visiting, QSet<QString> &visited)
{
    ...
    for (const QString &dependency : dependencies.value(id)) {
        if (!visitOperation(dependency, dependencies, visiting, visited)) {
            return false;
        }
    }
    visiting.remove(id);
...
{
    if (risk == VibeCutToolRisk::ReadOnly) {
        return false;
    }
    if (confirmationRequired || risk == VibeCutToolRisk::Irreversible) {
        return true;
    }
    if (mode == VibeCutTrustMode::Off) {
        return true;
    }
...
QJsonObject VibeCutEditPlan::toJson() const
{
    ...
    for (const VibeCutPlanOperation &operation : operations) {
        operationArray.append(operation.toJson());
    }
    return QJsonObject{
        {QStringLiteral("id"), id},
...
VibeCutPlanValidation VibeCutEditPlan::validate() const
{
    ...
    if (id.trimmed().isEmpty()) {
        result.errors.append(QStringLiteral("plan id is required"));
    }
    if (objective.trimmed().isEmpty()) {
        result.errors.append(QStringLiteral("plan objective is required"));
    }
    if (operations.isEmpty()) {
        result.errors.append(QStringLiteral("plan must contain at least one operation"));
    }
    ...
    for (const VibeCutPlanOperation &operation : operations) {
        ...
        if (ids.contains(operation.id)) {
            result.errors.append(QStringLiteral("duplicate operation id: %1").arg(operation.id));
        }
        ids.insert(operation.id);
        if (operation.toolName.trimmed().isEmpty()) {
            result.errors.append(QStringLiteral("operation %1 has no tool").arg(operation.id));
        }
    ...
    for (const VibeCutPlanOperation &operation : operations) {
        for (const QString &dependency : operation.dependsOn) {
            if (dependency == operation.id) {
                result.errors.append(QStringLiteral("operation %1 depends on itself").arg(operation.id));
            } else if (!ids.contains(dependency)) {
                result.errors.append(QStringLiteral("operation %1 depends on unknown operation %2").arg(operation.id, dependency));
            }
        }
    }
...
bool VibeCutEditPlan::requiresConfirmation(const QHash<QString, VibeCutToolPolicy> &policies, VibeCutTrustMode mode) const
{
    for (const VibeCutPlanOperation &operation : operations) {
        ...
        if (policy.value().requiresConfirmation(mode)) {
            return true;
        }
    }
    return false;
