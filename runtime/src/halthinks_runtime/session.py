...
from .contracts import EditPlan, PlanValidationError, validate_plan
from .policy import ToolPolicy, TrustMode, plan_requires_confirmation
from .protocol import AdapterClient, Envelope, ProtocolError, request
from .revision import RevisionError, RevisionGate
...
class RuntimeSession:
    ...
    def accept_authorization(self, envelope: Envelope | Mapping[str, Any]) -> dict[str, Any]:
        ...
        for raw in approved:
            approved_ids.append(_bounded_string(raw, "approved operation id"))
        ...
        *,
        ...
        try:
            for operation_id in self.execution_order:
                ...
                invoke_response = self._exchange(adapter, invoke)
                ...
                if invoke_payload.get("started") is True:
                    ...
                    self._wait_for_job(adapter, job_id, gate)
                ...
                verify_response = self._exchange(adapter, verify)
    ...
    def _wait_for_job(self, adapter: AdapterClient, job_id: str, gate: RevisionGate) -> None:
    ...
    def _abort_best_effort(self, adapter: AdapterClient, reason: str) -> None:
        ...
        if plan is None:
            return
        payload: dict[str, Any] = {"plan_id": plan.id, "reason": reason[:16_384] or "runtime aborted"}
        if self.authorization_id:
            payload["authorization_id"] = self.authorization_id
        try:
            adapter.exchange(request("abort_plan", payload))
        except Exception:
            pass
        self._clear_authorization()
