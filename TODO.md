...
- [ ] Pacing/narrative/continuity descriptive-analysis smoke with semantic fallback and no normative execution thresholds.
- [ ] Editorial agreement/evaluation-case/blinded-review smoke with exact context/proposal binding.
- [ ] Retrieval/duplicate evaluator smoke with deterministic top-k metrics and order-independent pair identity.
...
- [ ] Retrieval/duplicate agreement metrics cannot become semantic/duplicate truth or automatic execution authority.
...
- [x] `retrieval_ranking_evaluate`: explicit-reference precision@k, recall@k, AP@k, binary nDCG@k, reciprocal rank and full-list recall.
- [x] Deterministic synthetic retrieval metric corpus `tests/dataset/vibecut/retrieval_ranking_cases.json`.
- [x] `duplicate_ranking_evaluate`: order-independent duplicate-pair identity plus retrieval-ranking agreement and fusion/evidence diagnostics.
- [x] Deterministic synthetic duplicate metric corpus `tests/dataset/vibecut/duplicate_ranking_cases.json`.
...
## Pacing / narrative / continuity analysis — DESCRIPTIVE FOUNDATION LANDED
...
- [x] `rough_cut_continuity_analyze`: structural chronology reversals, overlapping source ranges, repeated full-text hashes, source-provenance changes and ranked positive source-gap candidates.
- [x] Pacing/narrative/continuity outputs apply no absolute good/bad threshold and remain derived analysis only.
...
- [x] Generic retrieval/duplicate agreement evaluators and deterministic synthetic metric corpora are source-landed.
...
- [ ] Build representative retrieval relevance and duplicate-pair reference datasets from real projects.
...
- [x] Pure source fixtures for rough-cut context integrity, objective relevance, alternative comparison, highlights, B-roll, pacing, narrative and continuity authority/semantics.
...
- [x] Generic retrieval ranking evaluator + deterministic synthetic metric corpus.
- [x] Order-independent duplicate-pair ranking evaluator + deterministic synthetic metric corpus.
- [ ] Representative editorial benchmark/review corpus; current deterministic cases validate plumbing/metrics only.
- [ ] Representative retrieval/duplicate benchmark references from real projects.
...
Phase 4 source foundations are substantially present; R3 requires runtime verification, representative retrieval-quality/duplicate-ranking evaluation, stale-index hardening and any remaining useful camera/composition evidence.

## R4 — Editorial copilot
Proposal-only rough-cut/highlight/B-roll plus pacing/narrative/continuity/evaluation infrastructure is source-landed. R4 additionally requires representative evaluation cases/blinded reviews and Phase 6 measurement evidence; this does not itself imply autonomous execution.
...
3. **Quantify retrieval/duplicate quality on real projects:** build explicit relevance/pair references and score current rankings with `retrieval_ranking_evaluate` and `duplicate_ranking_evaluate`; do not let synthetic metric fixtures masquerade as quality evidence.
4. **Compare proposal/provider versions on identical frozen cases/evidence budgets** using reference agreement + human-review distributions + descriptive pacing/narrative/continuity measurements, not one opaque quality score.
